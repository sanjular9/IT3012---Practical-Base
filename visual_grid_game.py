...
class VisualGridHuntGame:
    ...
    def __init__(self, width=10, height=10, num_food=10, num_opponents=2, custom_walls=None):
        ...
        num_traps = max(1, min(5, (self.width * self.height) // 10))
        opponent_positions = {tuple(op) for op in self.opponents}
        while len(self.toxic_traps) < num_traps:
            tx = random.randint(0, self.width - 1)
            ty = random.randint(0, self.height - 1)
            trap_pos = (tx, ty)
            if trap_pos != (0, 0) and trap_pos not in self.walls and trap_pos not in self.food_positions and trap_pos not in opponent_positions:
...
class GridGameGUI:
    ...
    def draw_grid(self):
        ...
        for tx, ty in self.env.toxic_traps:
            ...
            self.canvas.create_rectangle(x1, y1, x1 + self.cell_size * 0.7, y1 + self.cell_size * 0.7, fill="#8b5cf6",
                                         outline="#7c3aed")
